...
export default function PaymentTypesTable({ paymentTypes }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (editingItem) {
            put(route('settings.parameters.payment_types.update', editingItem.id), {
                onSuccess: () => closeModals(),
            });
        } else {
            post(route('settings.parameters.payment_types.store'), {
                onSuccess: () => closeModals(),
            });
        }
    };
    ...
    const deleteItem = (item) => {
        if (confirm('¿Eliminar este tipo de pago?')) {
            destroy(route('settings.parameters.payment_types.destroy', item.id));
        }
    };
